...
- Cloudflare API failures (HTTP errors and `success: false` API responses) now
  ...
  `Cloudflare API request failed: GET /accounts/<account>/ai-gateway/gateways/<id>: <message>`.
  This applies to every method on this model, since they all share the same
  request helper.
- Required identifier arguments (`id`, `name`, `gateway_id`, `job_id`,
  `item_id`, `version_id`, `model_name`, `provider`) are now validated as
