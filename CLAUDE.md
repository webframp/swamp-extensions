...
- **A version bump is coupled to three other things in the same file tree — miss any and CI fails.** When you bump a model's CalVer version, you must also: (1) **append** a new entry to the model's `upgrades:` array whose `toVersion` equals the new version (never relabel the last existing entry in place — that destroys a historical migration step and mis-attaches its `upgradeAttributes`; append with an identity migration `(old) => old` for dependency/license bumps), and (2) update any **exact-literal test assertion** of the form `assertEquals(model.version, "<old>")` to the new version. Pattern-based assertions (`assertMatch(model.version, /regex/)`) need no change. `deno check`/`lint` pass while these drift — only `swamp extension quality` catches the broken chain, and only `deno test` catches the stale assertion. The `@webframp/extension-maintenance` sweep (2026.08.28.2+) handles all three automatically; hand bumps must do them by hand.
