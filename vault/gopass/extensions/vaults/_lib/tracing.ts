/**
 * OpenTelemetry helpers for the gopass vault provider.
 *
 ...
 * `exception.message`, and a stack trace. A gopass error message is the CLI's
 * stderr, which the extension does not control, so `recordException` here would
...
const INSTRUMENTATION_NAME = "@webframp/gopass";
...
export const Attr = {
  ...
  /** Configured gopass store or mount, when one is set. */
...
/**
 * Sets a count on the active span, if there is one.
 *
 * Used where the count is only known after the work is done and the caller
 * would otherwise have to hold a span reference.
 */
export function recordCount(key: AttrKey, value: number): void {
 ...
 *
 * The host publishes thrown error messages to the trace backend, and a CLI that
 * rejects a value often echoes that value back on stderr. Where the extension
 * holds the secret it just submitted, it can strip it exactly rather than guess
 * at patterns.
 *
