/**
 * Schema fetcher — downloads and caches the fal.ai Platform APIs OpenAPI spec.
 *
 * Fetches from the live fal.ai endpoint, caches locally in .cache/ for 24h,
 * and parses the JSON into a typed intermediate representation.
 *
 * fal.ai's spec is OpenAPI 3.1: nullability is expressed as `type: [T, "null"]`
 * rather than the 3.0-style `nullable: true` sibling keyword. Both forms are
 * modeled on SchemaObject.type so downstream consumers (type_mapper.ts) can
 * handle either.
 */
...
export interface SchemaObject {
  /** OpenAPI 3.0 uses a single string; 3.1 permits an array (e.g. ["string", "null"]). */
  type?: string | string[];
  format?: string;
  description?: string;
  ...
  anyOf?: SchemaObject[];
  $ref?: string;
  /** OpenAPI 3.0-style nullable sibling keyword. */
  nullable?: boolean;
  readOnly?: boolean;
  writeOnly?: boolean;
  default?: unknown;
...
/** Fetch or load from cache the fal.ai OpenAPI spec */
export async function fetchSchema(
  ...
  console.log(`Fetching fal.ai OpenAPI spec from ${SCHEMA_URL}...`);
