/**
 * fal.ai OpenAPI → swamp extension code generator.
 *
 * Usage:
 *   deno task generate              # Generate all configured services
 *   deno task generate -- --dry-run # Show what would be generated
 *   deno task generate -- --service assets,compute  # Generate specific services
 *
 * @module
 */
...
async function main() {
  ...
  console.log(`\n🔧 fal.ai Extension Code Generator`);
  console.log(
    `   Version: ${
  ...
  console.log(`📥 Fetching fal.ai OpenAPI spec...`);
  ...
  if (groups.length === 0) {
    console.log(
      `⚠️  No operations matched the configured service tags/prefixes.`,
    );
    console.log(
      `   Check your tags/pathPrefixes in config.ts against the actual spec.`,
    );
    Deno.exit(0);
  }

  // Two-pass plan/write — see cloudflare-codegen/main.ts for the rationale:
  // a rejected --version must never leave a half-written tree.
  ...
  for (const planned of plan) {
    ...
    const modelDir = join(extDir, "extensions", "models", "falai");
