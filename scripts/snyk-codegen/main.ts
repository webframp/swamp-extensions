/**
 * Snyk OpenAPI → swamp extension code generator.
 *
 * Usage:
 *   deno task generate              # Generate all configured services
 *   deno task generate -- --dry-run # Show what would be generated
 *   deno task generate -- --service projects,issues  # Generate specific services
 *
 * @module
 */
...
async function main() {
  ...
  // Fetch schema
  console.log(`📥 Fetching Snyk OpenAPI spec...`);
  ...
  for (const group of groups) {
    ...
    const modelDir = join(extDir, "extensions", "models", "snyk");
    ...
    if (opts.dryRun) {
      ...
      console.log(`        extensions/models/snyk/${modelFileName}`);
      console.log(`        extensions/models/snyk/${testFileName}`);
      console.log(`        extensions/models/snyk/_lib/api.ts`);
      continue;
    }
