/**
 * Datadog OpenAPI → swamp extension code generator.
 *
 * Usage:
 *   deno task generate              # Generate all configured services
 *   deno task generate -- --dry-run # Show what would be generated
 *   deno task generate -- --service monitors,incidents  # Generate specific services
 *
 * @module
 */
...
async function main() {
  ...
  // Fetch schema
  console.log(`📥 Fetching Datadog v2 OpenAPI spec...`);
  ...
  if (groups.length === 0) {
    console.log(`⚠️  No operations matched the configured service tags.`);
    console.log(
      `   Check your tags in config.ts against the actual spec tags.`,
    );
    Deno.exit(0);
  }
  ...
  for (const group of groups) {
    ...
    const modelDir = join(extDir, "extensions", "models", "datadog");
