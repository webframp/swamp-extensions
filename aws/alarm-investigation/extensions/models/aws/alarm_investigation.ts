/**
 * AWS CloudWatch alarm investigation and triage model.
 *
 * Enriches CloudWatch alarms with metric activity, SNS subscription data,
 * state-change history, and a verdict classifying each alarm as one of:
...
/** Zod schema for SNS topic subscription information. */
...
/** Zod schema for the global arguments accepted by this model. */
...
/** Zod schema for an enriched alarm detail resource. */
...
/** Zod schema for the triage summary resource. */
...
/** Classification verdict for an alarm's operational health. */
type Verdict =
  | "stale"
  | "silent"
  | "noisy"
...
/** Enriched alarm detail derived from the AlarmDetailSchema. */
type AlarmDetail = z.infer<typeof AlarmDetailSchema>;

/** SNS topic subscription info derived from the SnsTopicSchema. */
...
/**
 * CloudWatch alarm investigation model definition.
 *
 * Provides two methods (`investigate` and `triage`) that enrich CloudWatch
 * alarms with metric data, SNS subscription info, state-change history,
 * and a diagnostic verdict. Produces `alarm_detail` and `triage_summary`
 * resources.
 */
export const model = {
