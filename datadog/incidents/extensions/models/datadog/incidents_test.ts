...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      const resources = getWrittenResources();
      assertEquals(resources.length, 1);
      assertEquals(resources[0].specName, "incident_impact");
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      assertStringIncludes(req0.path, "/impacts/test-id-123");
      assertEquals(req0.headers["dd-api-key"], "test-api-key");
      assertEquals(req0.headers["dd-application-key"], "test-app-key");
...
Deno.test({
  name: "incidents model: list_incident_impacts surfaces API errors",
  // sanitizeResources: false — Deno.serve() listener outlives test scope
  sanitizeResources: false,
  fn: async () => {
